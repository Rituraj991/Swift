...
function ExpensesScreen({ params }) {
  ...
  const getBudgetInfo = async () => {
    const result = await db
      ...
      .where(eq(Budgets.id, budgetId))  // Use the unwrapped budgetId
      .groupBy(Budgets.id);
  ...
  const getExpensesList = async () => {
    const result = await db
      .select()
      .from(Expenses)
      .where(eq(Expenses.budgetId, budgetId))  // Use the unwrapped budgetId
  ...
  return (
    <div className="p-10">
      ...
      >
        ...
        <AddExpense
          budgetId={budgetId}  // Use the unwrapped budgetId
          user={user}
          refreshData={() => getBudgetInfo()}
        />
      </div>
